...
Enable it with `/handoff install --idle-timer` (also registers the session hook above). `--uninstall --idle-timer` removes just the timer; `--uninstall` alone removes both. `HANDOFF_IDLE_TIMER=0` turns it off without uninstalling; set it in the `env` block of Claude Code's `settings.json` or export it before launching Claude Code, since hooks inherit the environment Claude Code was launched with and exporting it in a shell mid-session has no effect. Tune with `HANDOFF_IDLE_MINUTES`, `HANDOFF_IDLE_MIN_BYTES` (default 2000, a transcript-size floor below which nothing arms), and `HANDOFF_IDLE_COOLDOWN_SECONDS` (default 120, so a fresh write doesn't immediately get re-armed).

If there was text typed in the input box when the timer fired, it is salvaged to `.handoff/<lane>.draft.txt` before the line is cleared to make room for the injected command — otherwise the injection would concatenate with the typed text and silently produce no handoff. A wrapped line is joined back into one line; a draft spanning several lines is salvaged in full and suppresses the injection entirely (`Ctrl+U` clears one line at a time, so the timer writes the file and stops rather than submit the leftovers as prose). When the pane is not showing the input box (no prompt marker), the timer injects nothing; a non-blank screen is written to the draft file under a header saying the marker was missing, and a blank one writes no file.
