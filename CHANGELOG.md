...
- `handoff/scripts/idle-timer.sh`, an opt-in `Stop` hook (`/handoff install --idle-timer`) that cancels and re-arms a detached timer at the end of every turn and, if the session is still idle after `HANDOFF_IDLE_MINUTES` (default 58), injects `/handoff` into the live pane so the progress file gets written before the ~60 minute prompt-cache TTL lapses and the next turn re-uploads the whole context at the write rate. It never triggers `/clear` — that stays the user's manual signal — and it salvages any text left typed in the input box to `.handoff/<lane>.draft.txt` before clearing the line to submit the command, since the alternative concatenates the injection with the typed text and silently produces no handoff. Requires `$TMUX_PANE` or `$HERDR_PANE_ID`; the tmux injection path is verified end to end against a live session, the herdr path is implemented by analogy and has not been confirmed against a real herdr pane. `handoff/scripts/install-hook.sh` gained the `--idle-timer` flag to register (or, with `--uninstall`, remove) just this hook without touching the `SessionStart` reload hook
...
### Fixed
- `handoff` keyed its progress file by repository (`.handoff/progress.md`), so two sessions sharing a repo (two panes, two worktrees) clobbered each other's file. It now writes `.handoff/<lane>.md` and `.handoff/<lane>.checks.md`, where `<lane>` resolves from `$HANDOFF_LANE`, then `$HERDR_PANE_ID`, then `$TMUX_PANE`, then falls back to the literal `progress` — a session with none of those set keeps writing `.handoff/progress.md`, unchanged from before. `$HANDOFF_LANE` is a manual override for naming a lane directly
